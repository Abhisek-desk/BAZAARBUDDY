...
export const inngest = new Inngest({ id: "groc-del" });
...
const checkLowStock = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const product=await step.run('feth-product',async()=>{
...
const sendMonthlyOffers=inngest.createFunction({
    ...
    const {deals,users}=await step.run("fetch-deals-and-users",async()=>{
        //Get top deiscounted products as featured deals
    ...
    //send inbatches of 10 to avoid overwheming mail server
    const batchSize=10;
...
//Auto - Assign Riders after 5 mins
...
const autoAssignRider=inngest.createFunction({
    ...
    const result= await step.run("assign-rider",async()=>{
        ...
        const busyOrders=await prisma.order.findMany({
            where:{status:{in:["Assigned","PAcked","Out for deivery"]},
    ...
    const availableRider=await prisma.deliveryPartner.findFirst({where:{isActive:true,id:{notIn:busyRiderIds as string[]}}})
      ...
      history.push({
        status:"Assigned",
        note:`Autp-assiggned to ${availableRider.name}`,
        timeStamp:new Date(),
      })
